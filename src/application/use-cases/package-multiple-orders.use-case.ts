...
import { PackagingRequestDto, PackagingResponseDto, PackagedOrderDto } from '../dto/packaging.dto';
...
export class PackageMultipleOrdersUseCase {
  ...
  async execute(dto: PackagingRequestDto): Promise<PackagingResponseDto> {
    ...
    for (const orderDto of dto.pedidos) {
      ...
      const packagedOrder: PackagedOrderDto = this.mapper.mapResultsToPackagedOrder(orderDto.pedido_id, results);
