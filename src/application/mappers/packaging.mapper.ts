...
import { PackagingRequestDto, OrderDto, ProductDto, PackagedBoxDto, PackagedOrderDto } from '@application/dto/packaging.dto';
...
export class PackagingMapper {
  mapOrderDtoToDomain(orderDto: OrderDto): Order {
    const products: Product[] = orderDto.produtos.map((p: ProductDto) => {
      return Product.create(p.produto_id, p.dimensoes.altura, p.dimensoes.largura, p.dimensoes.comprimento);
    });
  ...
  mapResultsToPackagedOrder(pedidoId: number, results: PackagingResult[]): PackagedOrderDto {
